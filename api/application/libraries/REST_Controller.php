...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        {
            $supported_formatsarray[] = $default_format;
        }
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
		...
		if(!$isProfilingEnabled){
        	...
        	elseif ($data !== NULL)
        	{
            	...
            	if (method_exists('json', 'to_' . $this->response->format))
            	{
    ...
    protected function _userLoginCheck( $token )
    {
        if( $token == '' )
        {     
        ...
        $user = $this->CommonModel->selectRowDataByCondition( $token , 'user');
        // print_r($user);exit;
    ...
    protected function _vendorLoginCheck( $token )
    {
        if( $token == '' )
        {     
        ...
        $this->load->model( 'CommonModel' );
        $user = $this->CommonModel->selectRowDataByCondition( $token , 'vendor');
        // print_r($user);exit;
     ...
     public function send_otp($msg,$mobile,$otp)
    {
        $curl       = curl_init();
        $authkey    = '295182AgvnI4kIyy5d85c2f8';
        // $msg        = $msg;
        $sender_id  = 'ONMDCN';
        // $mobile     = $mobile;
        // echo"https://control.msg91.com/api/sendotp.php?email=&template=&otp=".$otp."&otp_length=4&otp_expiry=&sender=".$sender_id."&message=".$msg."&mobile=".$mobile."&authkey=".$authkey."";
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://control.msg91.com/api/sendotp.php?email=&template=&otp=".$otp."&otp_length=4&otp_expiry=&sender=".$sender_id."&message=".$msg."&mobile=".$mobile."&authkey=".$authkey."",
    ...
    public function send_otp1($msg,$mobile,$otp)
    {
        $curl       = curl_init();
        $authkey    = '295182AgvnI4kIyy5d85c2f8';
        // $msg        = $msg;
